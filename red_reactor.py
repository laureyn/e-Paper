...
def verify_RedReactor_requirements():
	# ADC Default
	...
	try:
		...
		logging.error("RED REACTOR IS NOT Attached, exiting:", e)
		exit(1)
	except RuntimeError as e:
		logging.error("Failed to read from I2C bus: ", e)
		exit(2)
	else:
		logging.info("RED REACTOR Attached")
...
class RedReactor:
	...
	def __init__(self, measure_interval):
		...
		logging.info("RED REACTOR: Initialising battery manager")
		self.measure_interval = measure_interval
		logging.info("RED REACTOR: Battery read interval set to {}s".format(self.measure_interval))
	...
	def battery_reader(self):
		...
		while not self.stop_reader:
			...
			try:
				...
				logging.info("RED REACTOR: Measurement Range Error:\n", battery_error)
		...
		if self.shutdown:
			logging.info("Battery Monitor: Exiting on battery voltage warning")
			# Enable to force a system shutdown from here
			# subprocess.Popen(['sleep 5;sudo shutdown -r now'], shell=True)
		else:
			logging.info("Battery Monitor: Exiting on user request")